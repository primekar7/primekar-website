...
## Technical SEO
One H1/page; logical H2/H3; crawlable HTML text (no headings/service copy inside images/canvas — Blueprint p24); self-referencing canonicals; internal links (airport↔routes↔corporate↔fleet↔FAQ↔guides); breadcrumbs on service/route pages; OG + social metadata; unique title + meta description each page. XML sitemap = only final indexable canonical URLs (exclude 404/error/redirect/thin). `[VERIFICATION REQUIRED]` must never reach metadata/alt/structured data (v7 §23).

**Phase F (docs/page-template-review.md PT-012):** `src/app/sitemap.ts` and `src/app/robots.ts` implemented, generated from `src/lib/site-routes.ts` (a single list of routes that actually exist — updated as each page is built, never ahead of it). `robots.ts` allows all crawling and points at the sitemap. Both resolve against `NEXT_PUBLIC_SITE_URL`, so URLs are `localhost` until the hosting decision (F-015/OPEN-1) sets that variable in the deployment environment — expected on a pre-hosting feature branch, a production blocker until then.

**Phase F Service-schema decision (docs/page-template-review.md PT-008):** Service/TransportationService structured data is **explicitly deferred**, not silently omitted — entity details (address, areaServed, founding facts) remain unverified per OPEN-3/D-010, and v7 §24 requires verification before publishing schema, not just before publishing visible copy. Every service/route page emits `Organization` (site-wide, unchanged) + `BreadcrumbList` (page-specific, absolute URLs) only. Add `Service` once entity details are verified and re-validate with the Schema Markup Validator at that point. FAQPage schema is likewise not added — no documented non-Google consumer need exists yet, matching the existing rule above; visible FAQ content uses a semantic `<details>/<summary>` accordion regardless (docs/page-template-review.md PT-016).
