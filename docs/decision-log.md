...
## Phase E approval and Phase F scope (2026-07-20)

| ID | Date | Phase | Decision | Options considered | User-approved outcome | Affected | Status |
|---|---|---|---|---|---|---|---|
| D-019 | 2026-07-20 | D→E gate | **Phase D findings reviewed** — Airport service-page template and Banff route-page template reviewed against premium-brand quality, composition, content depth, performance, accessibility and SEO; 16 findings recorded (`docs/page-template-review.md` PT-001–PT-016; 2 Critical, 4 High, 7 Medium, 3 Low) | — | User approved all 16 findings in full, no rejections/deferrals | `docs/page-template-review.md` | Approved |
| D-020 | 2026-07-20 | E→F gate | **Phase F sequencing** — Part 1 (correct all 16 findings) → Part 2 (re-verify Airport/Banff against acceptance criteria, gate) → Part 3 (build remaining approved pages only after Part 2 passes) | Rebuild templates first vs. mass-produce remaining pages from the flawed Phase D files | Sequenced approach approved; explicit instruction **not** to copy the pre-Phase-E Banff markup to Canmore/Lake Louise/Jasper | Build order for Phase F | Approved |
| D-021 | 2026-07-20 | F | **Template architecture** — `ServicePage`/`RoutePage` typed-content components built (`src/components/templates/`), sharing a hero/FAQ/final-CTA shell while keeping page-specific middle content bespoke per page (no generic page-builder abstraction) | Fully generic section-list config vs. shared shell + bespoke `children` | Shared-shell approach approved implicitly via Part 2 acceptance pass (no rejection raised) | `src/components/templates/service-page.tsx`, `route-page.tsx` | Approved |
| D-022 | 2026-07-20 | F | **JS-budget revision recommendation** — after removing Base UI entirely (custom dropdown + native `<dialog>`), measured floor is ~161KB gzip (down from 227–257KB), still above the proposed 130KB. Recommending the user approve **~165KB gzip** as the realistic revised budget for this stack | Keep chasing 130KB (would require dropping React/Next) vs. propose a measured, justified revision | **Not yet approved — awaiting explicit user decision** (v7 §30 forbids self-approving a budget change) | `docs/performance-budget.md`, F-006 | **Pending** |
| D-023 | 2026-07-20 | F | **Privacy Policy / Terms and Conditions — escalated, not built.** The verbatim legal body text (docs/legal-content-audit.md) was never committed to this repo; only opening excerpts were captured in Phase A, and the full raw HTML was session-scoped and is no longer accessible. Fabricating or paraphrasing legal text was judged out of scope | (a) fabricate/paraphrase legal body text; (b) leave unbuilt and escalate; (c) re-fetch live text through a tool that alters wording (rejected — WebFetch summarizes rather than preserves verbatim) | (b) — escalated to the user. `/privacy/` and `/terms/` still 404 (branded 404, `prefetch={false}`) | Legal pages, Phase G/deploy gate | **Pending — hard blocker for Phase G/deploy** |
| D-024 | 2026-07-20 | F | **Playwright test-infrastructure defect found and fixed** — `webServer.command` was `pnpm dev`; both locally (with no server pre-started) and in CI (`reuseExistingServer` forced false), the entire e2e suite was testing the dev server, not the production build the pipeline actually ships. Fixed to `pnpm build && pnpm start` | Leave as-is (relies on a human always pre-starting `pnpm start` before testing) vs. make the config self-sufficient and correct | Fixed; `playwright.config.ts` now always tests the production build | `playwright.config.ts`, CI reliability | Approved (fix applied; not a decision requiring approval, but recorded since it changes test behavior project-wide) |

## Phase B–F checkpoint and visual-reset pause (2026-07-21)

| ID | Date | Phase | Decision | Options considered | User-approved outcome | Affected | Status |
|---|---|---|---|---|---|---|---|
| D-025 | 2026-07-21 | F→G gate | **Phase B–F preserved as technical/functional baseline; Phase G paused for a visual redesign.** The Phase G final-site-audit was interrupted mid-run in the prior session (token exhaustion) before `docs/final-site-audit.md` was ever created — no incomplete/partial Phase G document exists to exclude. Phase B–F functionality (routes, content, SEO, accessibility, tests, page templates) is confirmed working (typecheck/lint/format/build all pass; 24 routes prerender) and is checkpointed as-is. The current visual direction is **rejected** as too basic and below the approved premium-brand reference bar. A high-fidelity visual redesign will happen before Phase G resumes; Fable will be used for visual reference analysis and concept creation. The redesign must preserve existing routes, content, SEO, accessibility, testing and business rules (legal/claims restrictions from D-009/D-010/D-013 remain in force). No pull request, merge, or deployment is approved at this checkpoint | (a) resume/complete Phase G audit on the current visual direction; (b) checkpoint the functional baseline, pause Phase G, and redesign visuals first | (b) approved | All Phase B–F source, `docs/decision-log.md`, Phase G (paused), new branch `feature/primekar-visual-reset` | Approved |
